...
### 4.6 把 `setNeedsDisplay`、`contents`、`CATransaction` 和 `CADisplayLink` 串起来看

```mermaid
sequenceDiagram
    participant Event as "Source0 / Timer / Block"
    participant Main as "Main Thread"
    participant RL as "RunLoop"
    participant DL as "CADisplayLink"
    participant CA as "Core Animation"
    participant RS as "Render Server / GPU"
    participant Screen as "Screen"

    Event->>Main: 业务代码执行
    alt "同步绘制路线"
        Main->>Main: setNeedsDisplay
        Note over Main: 只是标脏，不会立刻 drawRect
    else "异步 contents 路线"
        Main->>Main: layer.contents = CGImage
        Note over Main: 只是修改 layer tree\n不会立刻上屏
    end

    Note over Main,CA: 修改 layer tree 会进入当前线程的隐式事务\n隐式事务不是 Source0，它只是记录本线程的 layer 改动
    Main-->>RL: 当前事件继续执行

    opt "如果业务自己创建了 CADisplayLink"
        RL->>DL: beforeCADisplayLinkDispatch / 回调
        DL->>Main: displayLink selector
    end

    RL->>CA: beforeCATransactionCommit\n本轮后半段 flush / commit
    Note over RL,CA: 在 runloop.png 里，可近似对应“线程将要休眠”前的 observer 阶段

    alt "本轮 commit 前已经 setNeedsDisplay"
        CA->>Main: 调用 display / displayLayer / drawRect
        Main-->>CA: 生成 backing store / 提供显示结果
    else "本轮 commit 前已经改好 contents"
        Note over CA: 这次 commit 直接把最新 layer tree 送出
    end

    CA->>RS: 提交 render tree
    RS->>Screen: 下一个 VSync 合成并显示
```

这张图对应 4 个结论：

- Core Animation 提交和 RunLoop 强相关。对照 [runloop.png](/Users/huchu/Desktop/test-swift-program/runloop.png)，工程上通常把它放在“线程将要休眠”前的 observer / `BeforeWaiting` 附近理解。
- `setNeedsDisplay` 和 `layer.contents = ...` 都不是“立刻刷新屏幕”的方法。前者是标脏，后者是修改 layer tree；真正把改动送去显示的是后面的 `CATransaction` commit。
- 如果你在**当前这轮 RunLoop 的 commit 点之前**调用了 `setNeedsDisplay` 或改了 `contents`，它就可能在**本轮后半段**提交；如果错过了这个点，就等**下一轮 RunLoop**。
- `CADisplayLink` 不是主线程 RunLoop 默认就给业务准备好的回调。它需要你手动创建并 `addToRunLoop:forMode:`；一个 RunLoop 可以同时挂多个 `CADisplayLink`，但每个 display link 只能属于一个 RunLoop。你这个 demo 里的 FPS 监控就是手动加在 `NSRunLoopCommonModes` 上的：[ADFPSLabel.m](/Users/huchu/Desktop/test-swift-program/test-async-draw-oc/ADFPSLabel.m:27)
...
```mermaid
flowchart TD
    A["UIView / CALayer 调用 setNeedsDisplay"] --> B["当前 RunLoop 后半段\nCATransaction commit"]
    B --> C["进入当前 layer 的 display"]

    C --> D{"当前 layer 是否自己重写了 display?"}

    D -->|"是"| E["自定义 CALayer.display\n例如 YYAsyncLayer"]
    E --> F["自定义显示接管\n通常自己生成 bitmap / contents"]
    F --> Z["提交 render tree\nRender Server / GPU 合成显示"]

    D -->|"否"| G["默认 CALayer.display"]
    G --> H{"delegate 是否接管 displayLayer: ?"}

    H -->|"是"| I["layer.delegate displayLayer:"]
    I --> J["自定义显示结果\n通常直接设置 layer.contents"]
    J --> Z

    H -->|"否"| K["系统默认 backing store 绘制流程\n创建 CGContext backing store"]
    K --> L{"delegate 是否实现 drawLayer:inContext: ?"}

    L -->|"是"| M["layer.delegate drawLayer:inContext:"]
    M --> N["如果是 UIView-backed layer\n通常进一步到 drawRect:"]
    N --> O["得到 backing store"]
    O --> Z

    L -->|"否"| P["CALayer drawInContext:"]
    P --> O
```

这张图最关键的理解是：

- `displayLayer:` 是**显示接管点**
- `drawLayer:inContext:` / `drawRect:` / `drawInContext:` 是**默认 backing store 绘制点**

也就是说：

- 如果 delegate 实现了 `displayLayer:`，通常就在这里把默认绘制流程截胡了，不再继续走 `drawLayer:inContext:` / `drawRect:` 那条系统默认路径
- 如果没有人接管 `displayLayer:`，才进入后面的默认 backing store 绘制流程
- 如果 layer 本身重写了 `display` 并自己处理显示逻辑，例如 `YYAsyncLayer`，那通常连 `UIView.displayLayer:` 这条回调链也不会再走
...
### 7.8 为什么不建议在 `drawRect:` 里硬改成异步绘制

这个问题面试里也很容易被追问：

- 技术上能不能在 `drawRect:` 里 `dispatch_async` 到后台，画完位图后再回主线程设置 `layer.contents`？
...
原因不在于“后台线程完全不能画”，而在于 **`drawRect:` 的职责边界不对**。

#### 7.8.1 `drawRect:` 是同步绘制执行点，不是显示接管点

系统调用 `drawRect:` 时，已经说明两件事：

- 这次显示准备走默认 backing store 绘制流程
- 系统希望你在当前回调里，利用这次同步 context 当场把内容画完

如果你在 `drawRect:` 里只是把任务扔到后台，然后立刻返回，那么：

- 当前这次 `drawRect:` 本身并没有完成它应有的同步绘制职责
- 你后面回主线程 `layer.contents = ...`，其实是在绕到另一条显示提交路径上

#### 7.8.2 `drawRect:` 当次的系统 context 不能拿去后台继续用

`drawRect:` 里通过 `UIGraphicsGetCurrentContext()` 拿到的 context，只在：
...
所以如果你真想后台绘制，就只能像异步方案那样：

1. 在后台线程里自己 `UIGraphicsBeginImageContextWithOptions`
2. 自己创建离屏 bitmap context
3. 画完后回主线程设置 `layer.contents`

但一旦这样做，你其实已经不再依赖 `drawRect:` 的那套上下文了。
...
1. `setNeedsDisplay`
2. 进入 `drawRect:`
3. 在这次同步绘制里产出 backing store

你却改成：

1. `drawRect:` 被调用
2. 不在当前 context 里完成绘制
3. 后台另起一套 bitmap 流程
4. 最后手动改 `layer.contents`

这会导致：

- 系统默认绘制流程被“空跑”了一次
- 你又额外绕到 `contents` 提交流程
- 语义上不是“完成这次 `drawRect:`”，而是“借 `drawRect:` 当触发器”

#### 7.8.4 更容易出现空白帧、旧内容停留或闪烁

因为 `drawRect:` 返回时，新位图往往还没准备好。
...
而 `displayLayer:` 或自定义 `CALayer.display` 本身就是“这次显示由我自己提供结果”的接管点，更适合做异步显示。

#### 7.8.5 最适合面试的结论

你可以直接这样回答：

> `drawRect:` 里理论上可以 dispatch 到后台生成位图，最后再回主线程设置 `layer.contents`，但这并不适合作为正式异步绘制方案。因为 `drawRect:` 是系统默认同步绘制执行点，系统调用它时希望你在当前 context 中立即完成绘制；如果你在里面再异步转走，本质上就是放弃了这次 `drawRect:` 的同步绘制职责，转而绕到另一条 `contents` 提交路径上。真正适合做异步绘制接管的点是 `displayLayer:` 或自定义 `CALayer.display`。`
...
```mermaid
flowchart TD
    A["调用 setNeedsDisplay"] --> B["UIView.setNeedsDisplay"]
    B --> C["view.layer setNeedsDisplay"]
    C --> D["进入当前 layer 的 display"]

    D --> E{"当前 layer 是谁?"}

    E -->|"默认 CALayer"| F["CALayer.display"]
    F --> G{"delegate 是否响应 displayLayer: ?"}
    G -->|"是"| H["UIView.displayLayer:"]
    H --> I["主线程只做接管"]
    I --> J["后台线程创建 bitmap context"]
    J --> K["后台 CGContext 绘制"]
    K --> L["主线程 layer.contents = CGImage"]
    G -->|"否"| M["走系统默认绘制流程"]
    M --> N["drawLayer:inContext: / drawRect: 等默认路径"]

    E -->|"自定义 YYAsyncLayer"| O["YYAsyncLayer.display"]
    O --> P["不走 UIView.displayLayer:"]
    P --> Q["向 delegate 拉取 AsyncDisplayTask"]
    Q --> R["后台线程创建 bitmap context"]
    R --> S["后台 CGContext 绘制"]
    S --> T["主线程 layer.contents = CGImage"]

    L --> U["Core Animation 提交"]
    N --> U
    T --> U
    U --> V["Render Server / GPU 合成"]
    V --> W["屏幕显示"]
```

这张图重点看两个区别：

- `displayLayer:` 路线是 **layer 回调 view**
- `layerClass + display` 路线是 **layer 自己把事情做完**

### 9.2.2 极简面试口述图

```mermaid
flowchart LR
    A["setNeedsDisplay"] --> B["默认 CALayer.display"]
    B --> C["UIView.displayLayer:"]
    C --> D["后台绘制 bitmap"]
    D --> E["主线程赋值 contents"]

    A --> F["YYAsyncLayer.display"]
    F --> G["后台绘制 bitmap"]
    G --> H["主线程赋值 contents"]
```
...
**第一条是 layer 调回 view；第二条是 layer 自己统一调度异步绘制。**
...
### 11.3 为什么系统默认不把所有 UI 组件都做成异步绘制

这个问题很适合用来区分“知道怎么做”和“知道为什么系统没默认这么做”。

先说结论：

**异步绘制不是默认更高级的实现，而是一种只在特定瓶颈下值得付出额外工程复杂度的交换。**

#### 11.3.1 不是所有卡顿都来自主线程绘制

很多页面卡顿并不是因为 `drawRect:` 太重，而可能来自：
...
#### 11.3.3 UIKit 更优先保证主线程语义稳定

UIKit 的默认模型一直是：

- view tree 主线程操作
- 布局、事件、动画和显示时序尽量一致
- `drawRect:` 的语义清晰：给你 context，你现在画完

如果把所有控件默认异步化，系统就必须替所有控件统一解决这些问题：
...
你当前 demo 里自己已经能看到这类工程成本：

- `ADAsyncCardView` 需要 sentinel 取消旧任务：[ADAsyncCardView.m](/Users/huchu/Desktop/test-swift-program/test-async-draw-oc/ADAsyncCardView.m:44)
- `YYAsyncLayer` 也要统一处理 task、队列和取消逻辑：[YYAsyncLayer.m](/Users/huchu/Desktop/test-swift-program/test-async-draw-oc/YYAsyncLayer.m:109)

#### 11.3.4 异步绘制会带来额外内存和时序成本

异步绘制通常意味着：

- 单独创建离屏 bitmap context
- 生成 `UIImage / CGImage`
- 可能短时间同时存在旧图和新图
- 高频更新时反复创建和释放位图
...
也就是说，它是用“更复杂的内存和时序模型”换“主线程更轻”。

#### 11.3.5 很多系统控件更需要状态立即正确，而不是后台慢慢画
...
#### 11.3.6 系统已经做了很多更通用的优化

UIKit / Core Animation 默认并不是完全没优化，而是优先使用更通用、风险更小的手段，例如：
...
这类优化可以覆盖大多数控件和页面，不需要让所有控件都背上异步绘制那套复杂性。

#### 11.3.7 异步绘制更适合“重型自定义组件”
...
也就是你当前这个 demo 模拟的这种场景。

所以更合理的工程策略通常是：

- 默认控件走系统实现
- 少数重型 view 再手动接管 `displayLayer:` 或自定义 `CALayer.display`

#### 11.3.8 面试时最稳的总结

你可以直接这样说：

> UIKit 默认不把所有控件都做成异步绘制，不是因为异步绘制没价值，而是因为它只对重型 CPU 栅格化场景收益明显，同时会引入状态快照、任务取消、内存峰值、时序一致性和线程安全等额外复杂度。大多数系统控件默认同步绘制已经够快，系统更优先保证主线程语义稳定和通用性；只有在复杂富文本、卡片列表这类自定义重绘成本很高的场景下，才值得业务手动接管 `displayLayer:` 或自定义 `CALayer.display` 来做异步绘制。
